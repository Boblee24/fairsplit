...
const EXPENSE_EVENT = parseAbiItem(
...
const MEMBER_EVENT = parseAbiItem(
...
type ExpenseLog = Awaited<
  ReturnType<typeof client.getLogs<typeof EXPENSE_EVENT>>
>[number];
type SettleLog = Awaited<
  ReturnType<typeof client.getLogs<typeof SETTLE_EVENT>>
>[number];
type MemberLog = Awaited<
  ReturnType<typeof client.getLogs<typeof MEMBER_EVENT>>
>[number];
type GroupCreatedLog = Awaited<
  ReturnType<typeof client.getLogs<typeof GROUP_CREATED_EVENT>>
>[number];
...
async function fetchLogs(fromBlock: bigint, toBlock: bigint) {
  const [expenseLogs, settleLogs, memberLogs, groupCreatedLogs] =
    await Promise.all([
      client.getLogs({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        allExpense = allExpense.filter((l) =>
          groupIdSet.has(l.args.groupId?.toString() ?? ""),
        );
        allSettle = allSettle.filter((l) =>
          groupIdSet.has(l.args.groupId?.toString() ?? ""),
        );
        allMember = allMember.filter((l) =>
          groupIdSet.has(l.args.groupId?.toString() ?? ""),
        );
        ...
        const creatorTxHashes = new Set(
          allGroupCreated
            .filter(
              (l) => l.args.creator?.toLowerCase() === address!.toLowerCase(),
            )
        ...
        for (const log of allExpense) {
          ...
          if (!payer || payer.toLowerCase() === address!.toLowerCase())
            continue;
        ...
        for (const log of allMember) {
          ...
          if (!member || member.toLowerCase() !== address!.toLowerCase())
            continue;
